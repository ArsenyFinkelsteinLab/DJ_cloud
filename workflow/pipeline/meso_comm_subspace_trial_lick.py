...
def get_partition_by_lick(F,imaging_frame_rate,key,num_pieces):
    ...
    for p in range(num_pieces):
        ...
        for i_tr in range(num_trials):
            ...
            # np.concatenate((F_piece,tmp),axis=0)

        # F_piece = np.concatenate(F_piece,axis=0)
...
@schema
class CommSubspaceTrialLick(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    ...
    before_lick_flag     : int                          # frame index in trial
    ---
    r2            : blob
    """
    ...
    def make(self, key):
        ...
        for time_bin in time_bin_vector:
            ...
            num_pieces = 14
            F_source_partitioned = get_partition_by_lick(F_source_binned,imaging_frame_rate,key,num_pieces)
            F_target_partitioned = get_partition_by_lick(F_target_binned,imaging_frame_rate,key,num_pieces)
