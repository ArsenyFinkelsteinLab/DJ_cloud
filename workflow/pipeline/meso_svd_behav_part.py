...
@schema
class ROISVDBehavPartition1(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    -> img.ROI
    ...
    @property
    def key_source(self):
        return (exp2.SessionEpoch & img.ROIdeltaF & img.Mesoscope) - exp2.SessionEpochSomatotopy
    ...
    def make(self, key):
        ...
        self2 = SVDSingularValuesBehavPartition1
        self3 = SVDTemporalComponentsBehavPartition1
        self4 = SVDSingularValuesBehavPartition2
        self5 = SVDTemporalComponentsBehavPartition2
        self6 = ROISVDBehavPartition2
        self7 = ROISVDSpontPartition1
        self8 = SVDSingularValuesSpontPartition1
        self9 = SVDTemporalComponentsSpontPartition1
        self.compute_SVD(self2, self3, self4, self5, self6, self7, self8, self9, key, rel_data1, flag_zscore, time_bin, thresholds_for_event, threshold_variance_explained, num_components_save)
    ...
    def compute_SVD(self, self2, self3, self4, self5, self6, self7, self8, self9, key, rel_data1, flag_zscore, time_bin, thresholds_for_event, threshold_variance_explained, num_components_save):
        ...
        for t in range(num_partitions):
            ...
            for threshold in thresholds_for_event:
                ...
                InsertChunked(roi_self, key_ROIs, 1000)

                svd_key = {**key, 'time_bin': time_bin, 'threshold_for_event': threshold, 'partition': t}
                sv_self.insert1({**svd_key, 'singular_values': s}, allow_direct_insert=True)
                key_temporal = [{**svd_key, 'component_id': ic, 'temporal_component': vt[ic], 'partition': t}
                                for ic in range(num_components_save)]
                tc_self.insert(key_temporal, allow_direct_insert=True)


@schema
class ROISVDBehavPartition2(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    -> img.ROI
    threshold_for_event  : double                       # threshold in zscore, after binning. 0 means we don't threshold. 1 means we take only positive events exceeding 1 std, 2 means 2 std etc.
...
@schema
class ROISVDSpontPartition1(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    -> img.ROI
    threshold_for_event  : double                       # threshold in zscore, after binning. 0 means we don't threshold. 1 means we take only positive events exceeding 1 std, 2 means 2 std etc.
...
@schema
class SVDSingularValuesBehavPartition1(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
...
@schema
class SVDTemporalComponentsBehavPartition1(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    component_id         : int                          
    threshold_for_event  : double                       # threshold in deltaf_overf
...
@schema
class SVDSingularValuesBehavPartition2(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    threshold_for_event  : double                       # threshold in deltaf_overf
    time_bin             : double                       # time window used for binning the data. 0 means no binning
...
@schema
class SVDTemporalComponentsBehavPartition2(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    component_id         : int                          
    threshold_for_event  : double                       # threshold in deltaf_overf
...
@schema
class SVDSingularValuesSpontPartition1(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    threshold_for_event  : double                       # threshold in deltaf_overf
    time_bin             : double                       # time window used for binning the data. 0 means no binning
...
@schema
class SVDTemporalComponentsSpontPartition1(dj.Computed):
    definition = """
    -> exp2.SessionEpoch
    component_id         : int                          
    threshold_for_event  : double                       # threshold in deltaf_overf
